            ...
            while token is None:
                ...
                if counter > 5:
                    raise AssertionError(
                        f"Activation token is not recieved after {n} attempts"
                    )
...
class AccountHelper:
    ...
    async def register_new_user(self, login: str, password: str, email: str):
        ...
        resp_acc = await self.dm_account_api.account_api.post_v1_account(
    ...
    async def user_login(
        ...
        if validate_headers:
            assert resp_acc_login.headers["x-dm-auth-token"], (
                "Token has not been recieved"
            )
    ...
    async def get_user_info(self, token=None, validate_response: bool = True, **kwargs):
        ...
        resp_acc = await self.dm_account_api.account_api.get_v1_account(
            validate_response=validate_response, **kwargs
        )
    ...
    async def user_logout(self, token=None, **kwargs):
        ...
        resp_logout = await self.dm_account_api.login_api.delete_v1_account_login(
            **kwargs
        )
    ...
    async def user_logout_all(self, token=None, **kwargs):
        ...
        resp_logout_all = (
            await self.dm_account_api.login_api.delete_v1_account_login_all(**kwargs)
        )
    ...
    async def authenticate_client(self, login: str, password: str):
        resp_login = await self.user_login(
            login=login, password=password, validate_response=False
        )
    ...
    async def _get_activation_token_by_login(self, login):
        ...
        return await self._get_token_by_login(
            login=login, link_field="ConfirmationLinkUrl"
        )
    ...
    async def _get_reset_password_token_by_login(self, login):
        ...
        return await self._get_token_by_login(
            login=login, link_field="ConfirmationLinkUri"
        )
